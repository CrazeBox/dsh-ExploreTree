...
.rt-legend{position:absolute;right:10px;bottom:34px;display:flex;flex-wrap:wrap;gap:4px 10px;max-width:200px;padding:6px 8px;background:rgba(247,245,240,.9);border:1px solid #e3e1da;border-radius:8px;font-size:10px;color:#8a8578;backdrop-filter:blur(2px);pointer-events:none}
...
.rt-foot{flex:none;padding:6px 12px;border-top:1px solid #ece9e2;color:#9b968a;font-size:11px;display:flex;justify-content:space-between;align-items:center;background:#f7f5f0}
`;
			...
			return jsxs("div", {
				...
				children: [
						...
						children: [
							trees.length > 1
								? jsx("select", {
									...
									children: trees.map((tree) => jsx("option", {
										value: tree.treeId,
										title: tree.topic,
										children: `${truncate(tree.topic, 26)}（${tree.nodes.length}）`
									}, tree.treeId))
								})
								: jsx("span", { className: "rt-head-title", title: current !== null ? current.topic : "", children: current !== null ? current.topic : "研究进度树" }),
							jsx("button", {
								className: "rt-hbtn",
								type: "button",
								title: "缩小（也可 Ctrl+滚轮）",
					...
					jsx("div", {
						className: "rt-legend",
						"aria-label": "图例",
						children: Object.entries(STATE_LABELS).map(([state, label]) => jsxs("span", {
					...
					jsxs("div", {
						...
						children: [
							...
							jsx("span", { children: nodes.length > 0 ? `${nodes.length} 个节点${focused ? " · 滚轮缩放中 · Esc 退出" : " · 点面板聚焦后滚轮缩放"}` : "" })
						]
					})
				]
			});
