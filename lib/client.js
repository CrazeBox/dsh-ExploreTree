...
// 未定局节点虚线边框（卡住/没走完/未开始）、失败节点 ✕ 角标（此路不通）、角落图例、
// 当前节点高亮自动滚动；1.5s 轮询网关 RPC 实时刷新。
...
.rt-hbtn.on{border-color:#d97757;color:#d97757;background:rgba(217,119,87,.1)}
...
.rt-node.rt-emptyish .rt-node-rect{opacity:.55}
.rt-node.rt-emptyish .rt-node-title{fill:#b5b0a4}
.rt-node.rt-emptyish .rt-node-desc{fill:#c8c4b8}
...
.rt-tooltip.pinned h5{padding-right:48px}
.rt-editbtn{position:absolute;top:4px;right:26px;width:18px;height:18px;border:none;background:transparent;color:#9b968a;cursor:pointer;font-size:12px;line-height:1;border-radius:5px;padding:0}
.rt-editbtn:hover{background:#f0ede6;color:#3d3929}
.rt-edit-label{display:block;margin:2px 0 2px;color:#9b968a;font-size:10px}
.rt-edit-input{width:100%;margin:0 0 6px;padding:4px 6px;border:1px solid #ddd9d0;border-radius:6px;font-size:12px;color:#3d3929;background:#fff}
.rt-edit-area{width:100%;min-height:64px;margin:0 0 6px;padding:4px 6px;border:1px solid #ddd9d0;border-radius:6px;font-size:12px;color:#3d3929;background:#fff;resize:vertical;font-family:inherit;line-height:1.5}
.rt-edit-row{display:flex;justify-content:flex-end;gap:6px;margin-top:2px}
.rt-edit-row button{border:1px solid #ddd9d0;border-radius:6px;background:transparent;color:#6f6a5c;font-size:11px;line-height:1;padding:4px 10px;cursor:pointer}
.rt-edit-row button:hover{background:#f0ede6;color:#3d3929}
.rt-edit-row .save{border-color:#d97757;color:#d97757}
.rt-edit-row .save:hover{background:rgba(217,119,87,.1)}
.rt-edit-msg{font-size:10px;color:#c0392b;margin-top:4px}
...
.rt-legend .rt-ln{display:inline-block;width:22px;height:5px;margin-right:4px;vertical-align:middle}
...
`;
					...
					Promise.resolve()
						...
						.then((result) => {
							...
							if (result !== null && typeof result === "object" && result.ok === true && Array.isArray(result.value?.trees)) {
								setState({ status: "ready", trees: result.value.trees, storeRoot: result.value.storeRoot ?? null, error: null });
							} else {
		...
		// 结论语义 = 「线的命运」（科研口径）：
		//   success 走通（拿到结论） / failed 走不通（被否证，与成功同为科研产出）
		//   abandoned 没走完 / blocked 暂时卡住 —— 前两者是"有结论"，后两者是"未定局"
		const STATE_LABELS = {
			success: "走通",
			failed: "走不通",
			blocked: "卡住",
		...
		/** 节点副标题：这个节点在干什么（desc 优先；无 desc 时回退原因/类型文案）。 */
		function nodeDesc(node) {
			if (typeof node.desc === "string" && node.desc !== "") return node.desc;
			if (node.conclusion !== null && node.reason !== null && node.reason !== "") return node.reason;
			if (node.type === "plan" && node.reason !== null && node.reason !== "") return node.reason; // 计划自动汇总
			if (node.conclusion === "failed") return "此路不通";
			if (node.conclusion === "abandoned") return "没走完";
			if (node.conclusion === "blocked") return "暂时卡住";
			if (node.conclusion === "success") return "走通";
			if (node.status === "running") return "进行中…";
			if (node.status === "ended" && node.conclusion === null) {
				// 无内容的自动节点（子代理/工作流/任务/技能）显示类型文案，避免空壳占位感
				if (node.type === "subagent" || node.type === "workflow" || node.type === "job" || node.type === "skill") return typeLabel(node);
				return "已结束";
			}
		...
		// ── 分层树布局（direction: h=根在左向右展开 / v=根在顶向下展开） ────────
		const NODE_W = 156;
		const NODE_H = 46;
		const LEVEL_GAP = 200;
		const V_GAP = 26;
		...
		function computeLayout(nodes, collapsed, direction) {
			...
			const walk = (node, depth) => {
				...
				if (kids.length === 0) {
					const p = vertical
						? { x: cursor, y: PAD + depth * LEVEL_GAP, w: NODE_W, h: NODE_H }
						: { x: PAD + depth * LEVEL_GAP, y: cursor, w: NODE_W, h: NODE_H };
					pos.set(node.id, p);
					cursor += vertical ? NODE_W + V_GAP : NODE_H + V_GAP;
					return;
				}
				for (const kid of kids) walk(kid, depth + 1);
				const centers = kids.map((kid) => pos.get(kid.id)[vertical ? "x" : "y"]);
				const mid = (Math.min(...centers) + Math.max(...centers)) / 2;
				const p = vertical
					? { x: mid, y: PAD + depth * LEVEL_GAP, w: NODE_W, h: NODE_H }
					: { x: PAD + depth * LEVEL_GAP, y: mid, w: NODE_W, h: NODE_H };
				pos.set(node.id, p);
			};
			for (const root of roots) walk(root, 0);
			const width = vertical
				? Math.max(cursor + PAD, PAD * 2 + NODE_W)
				: PAD * 2 + maxDepth * LEVEL_GAP + NODE_W;
			const height = vertical
				? PAD * 2 + maxDepth * LEVEL_GAP + NODE_H
				: Math.max(cursor + PAD, PAD * 2 + NODE_H);
			return { pos, width, height, roots };
		}

		function edgePath(parent, child, vertical) {
			if (vertical === true) {
				const px = parent.x + parent.w / 2;
				const py = parent.y + parent.h;
				const cx = child.x + child.w / 2;
				const cy = child.y;
				const bend = Math.max(24, (cy - py) / 2);
				return `M ${px} ${py} C ${px} ${py + bend}, ${cx} ${cy - bend}, ${cx} ${cy}`;
			}
		...
		function detailRows(node) {
			...
			if (typeof node.desc === "string" && node.desc !== "") rows.push(["说明", node.desc]);
			if (node.conclusion !== null) rows.push(["结论", STATE_LABELS[node.conclusion]]);
			if (node.reason !== null && node.reason !== "") rows.push([node.type === "plan" ? "汇总" : "原因", node.reason]);
		...
		/** 图例线型示意：SVG 小线段（虚线清晰可控，不依赖 CSS border）。 */
		function LineSample({ dashed, color }) {
			return jsx("svg", {
				className: "rt-ln",
				width: 22,
				height: 5,
				...
				children: jsx("line", {
					...
					stroke: color ?? "#9b968a",
					strokeWidth: 2.5,
					...(dashed === true ? { strokeDasharray: "5 3" } : {})
				})
			});
		}

		function DetailCard({ node, x, y, pinned, dim, onUnpin, innerRef, onEdit }) {
			if (node === null) return null;
			// 固定卡片内编辑标题/描述（只有固定卡片可编辑；悬停卡片是临时的）
			...
			const startEdit = () => {
				...
				setDraftDesc(node.desc ?? "");
				setMsg("");
				setEditing(true);
			};
			const save = () => {
				if (onEdit === void 0) return;
				// 只提交有变化的字段（避免误清空未编辑的 desc）
				const fields = {};
				if (draftTitle.trim() !== (node.title ?? "")) fields.title = draftTitle.trim();
				if (draftDesc.trim() !== (node.desc ?? "")) fields.desc = draftDesc.trim();
				...
				setMsg("");
				Promise.resolve()
					.then(() => onEdit(node.id, fields))
					.then((result) => {
						setBusy(false);
						if (result !== null && typeof result === "object" && result.ok !== true) {
							setMsg(`保存失败：${String(result.error ?? "未知错误")}`);
							return;
						}
						setEditing(false);
					}, (error) => {
			...
			return jsxs("div", {
				...
				children: [
					...
					editing
						? jsxs(Fragment, {
							children: [
								jsx("label", { className: "rt-edit-label", children: "标题" }),
								jsx("input", {
								...
								jsx("label", { className: "rt-edit-label", children: "说明（在这个节点干了什么）" }),
								jsx("textarea", {
									className: "rt-edit-area",
									value: draftDesc,
									maxLength: 2000,
									placeholder: "可留空",
								...
								msg !== "" ? jsx("div", { className: "rt-edit-msg", children: msg }) : null
							]
						})
						: jsxs(Fragment, {
							children: [
		...
		function TreePanel({ rpc, sessions }) {
			...
			const [patches, setPatches] = react.useState(() => new Map()); // 乐观编辑补丁 nodeId -> {title?, desc?}
			const [direction, setDirection] = react.useState(() => {
				try {
					const saved = localStorage.getItem("research-tree-direction");
					return saved === "v" ? "v" : "h";
				} catch { return "h"; }
			});
			react.useEffect(() => {
				try {
					localStorage.setItem("research-tree-direction", direction);
				} catch { /* 忽略 */ }
			}, [direction]);
			...
			// 当前视图只显示可见树（正常 or 已归档）；无可见树时回到空状态页
			const visibleTrees = trees.filter((t) => (showArchived ? t.archived === true : t.archived !== true));
			const current = selectedId !== null && visibleTrees.some((t) => t.treeId === selectedId)
				? visibleTrees.find((t) => t.treeId === selectedId)
				: visibleTrees[0] ?? null;
			const nodes = current !== null ? current.nodes : [];
			const currentId = current !== null ? current.currentNodeId : null;

			const layout = react.useMemo(() => computeLayout(nodes, collapsed, direction), [nodes, collapsed, direction]);
			...
			// ── 面板内编辑（乐观更新）：先本地叠加补丁，RPC 写回后端；
			// 下一次轮询快照到达后丢弃补丁（服务端数据已包含改动） ──
			const patchNode = (n) => {
				if (n === null || n === void 0) return n;
				const p = patches.get(n.id);
				return p === void 0 ? n : { ...n, ...p };
			};
			const saveNodeEdit = (nodeId, fields) => {
				setPatches((prev) => {
					const next = new Map(prev);
					next.set(nodeId, { ...(prev.get(nodeId) ?? {}), ...fields });
					return next;
				});
				return Promise.resolve()
					.then(() => rpc.call("/api", "researchTree/annotateNode", { args: { treeId: current.treeId, nodeId, ...fields } }))
					.then((result) => {
						if (result !== null && typeof result === "object" && result.ok !== true) {
							rollbackPatch(nodeId);
							return result;
						}
						return { ok: true };
					}, (error) => {
			...
			react.useEffect(() => {
				if (snapshot.status === "ready") setPatches(new Map()); // 新快照已含服务端改动
			}, [snapshot]);
			...
			// 面板操作：归档/恢复、删除当前树；操作后选中列表中相邻的树（无则空状态）
			const selectAdjacent = () => {
				const list = trees.filter((t) => (showArchived ? t.archived === true : t.archived !== true));
				const idx = list.findIndex((t) => t.treeId === current.treeId);
				const next = list[idx + 1] ?? list[idx - 1] ?? null;
				selectedTreeId.set(next !== null ? next.treeId : null);
			};
			const archiveCurrent = (archived) => {
				...
				Promise.resolve()
					...
					.then((result) => {
						...
						selectAdjacent(); // 立即选中相邻树（轮询刷新会纠正列表）
			...
			return jsxs("div", {
				...
				children: [
					jsxs("div", {
						...
						children: [
							(() => {
								// 有多个可见树，或存在归档树时显示下拉+归档切换；否则纯标题
								const hasArchived = trees.some((t) => t.archived === true);
								const showSelect = trees.filter((t) => t.archived !== true).length > 1 || hasArchived;
								if (!showSelect) {
									return jsx("span", { className: "rt-head-title", title: current !== null ? `${current.topic}\ntreeId: ${current.treeId}` : "", children: current !== null ? truncate(current.topic, 30) : "研究进度树" });
								}
								return jsxs(Fragment, {
									children: [
										jsx("select", {
											...
											children: (() => {
												const visible = trees.filter((t) => showArchived ? t.archived === true : t.archived !== true);
												if (visible.length === 0) {
													return jsx("option", { value: "", children: showArchived ? "（没有已归档的树）" : "（没有树）" }, "empty");
												}
												return visible.map((tree) => jsx("option", {
													value: tree.treeId,
													title: `${tree.topic}\ntreeId: ${tree.treeId}`,
										...
										jsx("button", {
											className: `rt-hbtn${showArchived ? " on" : ""}`,
											type: "button",
											title: showArchived ? "回到正常树列表" : `查看已归档的树（${trees.filter((t) => t.archived === true).length} 棵）`,
											onClick: () => {
												setShowArchived((v) => {
													const next = !v;
													selectedTreeId.set(null); // 切换视图后回到该视图最新树
													return next;
												});
											},
											children: showArchived ? "正常" : `已归档(${trees.filter((t) => t.archived === true).length})`
										})
									]
								});
							})(),
							jsx("button", {
								className: `rt-hbtn${direction === "v" ? " on" : ""}`,
								type: "button",
								title: direction === "v" ? "切换为横向布局（根在左）" : "切换为竖向布局（根在顶，深链更直观）",
								onClick: () => setDirection((d) => (d === "v" ? "h" : "v")),
								children: direction === "v" ? "⇅" : "⇄"
							}),
							jsx("button", {
								className: "rt-hbtn",
								type: "button",
									...
									: jsxs("svg", {
										...
										children: [
											edges.map((node) => {
												...
												return jsx("path", { className: "rt-edge", d: edgePath(parent, self, direction === "v") }, `e-${node.id}`);
											}),
											[...layout.pos.keys()].map((id) => renderNode(id))
										]
									})
					}),
					jsx("div", {
						...
						children: [
							...
							jsx("div", {
								...
								children: [
									jsx("b", { children: "线型" }),
									jsxs("span", { children: [jsx(LineSample, {}), "实线：有结论/进行中"] }, "ln-solid"),
									jsxs("span", { children: [jsx(LineSample, { dashed: true }), "虚线：未定局"] }, "ln-dash"),
									jsxs("span", { children: [jsx(LineSample, { dashed: true, color: "#d97706" }), "修订计划"] }, "ln-yellow")
								]
							})
						]
					}),
					pinned !== null ? jsx(DetailCard, {
						node: patchNode(pinned.node),
						x: pinned.x,
						y: pinned.y,
						pinned: true,
			...
			function renderNode(id) {
				...
				// 未定局（卡住/没走完/未开始）：虚线边框表达"悬而未决"
				const isOpen = state === "abandoned" || state === "blocked" || state === "pending";
				// 角标：失败节点 ✕（此路不通）+ 计划对照 改/新
				const tags = [];
				if (node.conclusion === "failed") tags.push({ text: "✕", color: STATE_COLORS.failed });
				const relation = relationTag(node);
				if (relation !== null) tags.push(relation);
				// 无内容占位（自动节点结束但没拿到任何内容）：弱化显示（老数据兜底）
				const isEmptyish = isAuto && node.status === "ended" && node.conclusion === null
					&& (node.desc === null || node.desc === "") && (node.reason === null || node.reason === "")
					&& !isCurrent && !isPinned;
				return jsxs("g", {
					className: `rt-node${state === "running" ? " rt-node-pulse" : ""}${isEmptyish ? " rt-emptyish" : ""}`,
					...
					children: [
						jsx("rect", {
							...
							// 虚线 = 未定局（计划"还没落地"/卡住/没走完/未开始）；实线 = 有结论或进行中；
							// 当前节点也虚线高亮（比计划更粗的段）
							strokeDasharray: isPlan || isOpen || isCurrent ? (isCurrent && !isPlan && !isOpen ? "7 3" : "5 4") : void 0
						}),
						jsx("text", {
						...
						tags.length > 0
							? tags.map((tag, i) => {
								...
								return jsxs(Fragment, {
									children: [
										jsx("circle", { className: "rt-node-tag", cx: tx, cy: 12, r: 8, stroke: tag.color, strokeWidth: 1 }),
										jsx("text", { className: "rt-node-tag-text", x: tx, y: 15, textAnchor: "middle", fill: tag.color, children: tag.text })
									]
								}, `tag-${i}`);
							})
							: null,
