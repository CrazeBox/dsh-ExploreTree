...
// 职责：
//   1. 注册模型工具 tree_node：计划树、开分支、记结论（走通/走不通/没走完/卡住 + 原因）。
...
class TreeStore {
	 ...
	 *  额外：启动时进程里的子代理必然已死——所有 running 的子代理/工作流/任务
	 *  节点标记"进程中断"（孤儿清理），配对表清空。 */
	reconcileAll() {
		for (const tree of this.trees.values()) {
			let modified = false;
			// 1) 孤儿清理：running 的自动执行节点 = 上次进程中断的遗留
			for (const node of tree.nodes) {
				if (node.status === "running" && (node.type === "subagent" || node.type === "workflow" || node.type === "job" || node.type === "skill")) {
					node.status = "ended";
					node.conclusion = "abandoned";
					node.reason = "进程中断（未完成）";
					node.endedAt = node.endedAt ?? nowIso();
					modified = true;
				}
			}
			if (Array.isArray(tree.pendingSubagents) && tree.pendingSubagents.length > 0) {
				tree.pendingSubagents = []; // 进程重启后这些 end 事件不会再来了
				modified = true;
			}
			const root = this.rootNode(tree);
			if (root !== null && root.conclusion !== null) {
				if (this.settleTree(tree)) modified = true;
			} else {
				// 2) 静态收尾：plan/root 的子节点全部静止时收尾
				let changed = true;
				while (changed) {
					changed = false;
					for (const node of tree.nodes) {
	...
	summarizePlan(tree, planNode) {
		...
		const parts = [];
		if (counts.success > 0) parts.push(`${counts.success} 走通`);
		if (counts.failed > 0) parts.push(`${counts.failed} 走不通`);
		if (counts.abandoned > 0) parts.push(`${counts.abandoned} 没走完`);
		if (counts.blocked > 0) parts.push(`${counts.blocked} 卡住`);
...
class ResearchTreePlugin extends TypertRemoteService {
	...
	constructor(ctx, config = {}) {
		...
		this.childEvents = new Map(); // childSessionId -> 事件摘要数组（子代理节点任务/过程补全用）
		this.activeWorkflow = new Map(); // workflowId -> { treeId, nodeId }
		this.recentSessionId = null; // 最近见过且有树的会话（workflow 事件无会话，尽力归属）
		// ── 自动记录员配置（树落后对话时后台自动补记） ──
		this.scribe = {
			enabled: cfg.scribeEnabled !== false,
			provider: typeof cfg.scribeProvider === "string" && cfg.scribeProvider !== "" ? cfg.scribeProvider : "deepseek-official",
			model: typeof cfg.scribeModel === "string" && cfg.scribeModel !== "" ? cfg.scribeModel : "deepseek-v4-flash",
			lagMs: Number.isFinite(cfg.scribeLagMs) && cfg.scribeLagMs > 0 ? cfg.scribeLagMs : 10 * 60 * 1000, // 树落后阈值
			cooldownMs: Number.isFinite(cfg.scribeCooldownMs) && cfg.scribeCooldownMs > 0 ? cfg.scribeCooldownMs : 5 * 60 * 1000, // 两次补记最小间隔
			buffer: new Map(), // sessionId -> 摘要事件数组（本次进程启动后）
			lastScribeAt: 0,
			lastCheckAt: 0,
			inFlight: false
		};
	...
	/** 用户手动补充/修正一个节点的结论/原因/标题/描述（网关 SRC 契约：方法参数名 = 请求字段名）。 */
	annotateNode(treeId, nodeId, conclusion, reason, title, desc) {
		...
		if (typeof title === "string" && title.trim() !== "") node.title = title.trim().slice(0, 120);
		if (typeof desc === "string") node.desc = desc.trim() === "" ? null : desc.trim().slice(0, 2000);
	...
	installTool(ctx) {
		ctx.tools.register(defineTool({
			...
			description: [
				...
				"9. 中断恢复：研究被中断（如进程关闭）后重启，树保持原样、继续画在原来的树上",
				"   （永远不要为“继续研究”新建树，newTree 只用于重新研究或换主题）；",
				"   先 list/resume 找到原树，检查仍 running 的节点（即中断点），向用户汇报上次进度；",
				"   对上次中断未完成的节点用 conclude 标记（如 abandoned + reason=上次中断），",
				"   再从该节点或父级 start 继续。",
				"纪律：并行推进多个分支时，start/plan-child 必须显式传 parentId 指定挂载分支；",
				"start 时给具体标题（如“尝试方法X”），不要与计划分支标题重复。",
				"title 写短标签（≤12 字，节点上截断显示）；详细过程（干了什么）写 desc，可长文本；",
			...
			parameters: {
				...
				title: {
					type: "string",
					description: "节点标题（plan-root / plan-child / start 必填）——写短标签（≤12 字，节点上截断显示），详细过程写 desc。"
				},
				desc: {
					type: "string",
					description: "节点描述（在这个节点干了什么，可长文本、可多行）——start / plan-child / annotate 可选；conclude 时用 annotate 补写。"
				},
				parentId: {
					type: "string",
				...
				status: {
					type: "string",
					enum: [...CONCLUSIONS],
					description: "conclude 的结论（线的命运，科研口径）：success 走通（这条线拿到了结论）/ failed 走不通（被否证，与成功同为科研产出，务必写清原因）/ abandoned 没走完（未得出结论就中断）/ blocked 暂时卡住（外部条件，非方法问题）。"
				},
				reason: {
					type: "string",
		...
		try {
			...
			switch (action) {
				...
				case "plan-child": {
					...
					const desc = typeof args.desc === "string" ? (args.desc.trim() === "" ? null : args.desc.trim()) : null;
					const node = {
						id: shortId("plan"),
						type: "plan",
						parentId: parent.id,
				...
				case "annotate": {
					...
					if (typeof args.desc === "string") node.desc = args.desc.trim() === "" ? null : args.desc.trim();
	...
	subscribe(ctx) {
		...
		ctx.on("subagent/start", (info) => {
			try {
				...
				// 子代理会话事件缓冲（结束时补全节点的任务/过程描述）
				this.childEvents.set(childId, []);
			} catch (error) {
				this.log(error);
			}
		});
		...
		ctx.on("subagent/end", (info) => {
			try {
				...
				if (node !== null) {
					...
					// 事件侧补全（子代理节点不再空壳）：
					// desc = 子会话首条 user 文本（任务）；reason = 最终输出截取（结果）
					if (node.desc === null || node.desc === "") {
						const buf = this.childEvents.get(childId) ?? [];
						const task = buf.find((s) => s.startsWith("[user")) ?? buf[0] ?? "";
						if (task !== "") node.desc = `任务：${task.replace(/^\[[^\]]*\]\s*/, "").replace(/\s+/g, " ").slice(0, 200)}`;
					}
					if (node.reason === null || node.reason === "") {
						const out = this.blockText(info?.lastAssistantMessage);
						if (out.trim() !== "") node.reason = `结果：${out.replace(/\s+/g, " ").trim().slice(0, 240)}`;
					}
					this.childEvents.delete(childId);
					this.store.touch(tree);
		...
		// ── 自动记录员：收集会话事件，检测树落后时后台补记 ──
		ctx.on("session/event", (session, event) => {
			try {
				const sessionId = String(session?.header?.id ?? "");
				if (sessionId === "") return;
				const summary = this.scribeEventSummary(event);
				if (summary === null) return;
				// 子代理会话事件 → 补全缓冲（子代理节点任务/过程描述用；不依赖 scribe 开关）
				const childBuf = this.childEvents.get(sessionId);
				if (childBuf !== void 0) {
					childBuf.push(summary);
					if (childBuf.length > 40) childBuf.splice(0, childBuf.length - 40);
				}
				if (!this.scribe.enabled) return;
				const list = this.scribe.buffer.get(sessionId) ?? [];
				list.push(summary);
				if (list.length > 80) list.splice(0, list.length - 80);
				this.scribe.buffer.set(sessionId, list);
				// 节流检查（最多每 30s 一次）
				const now = Date.now();
				if (now - this.scribe.lastCheckAt < 30000) return;
				this.scribe.lastCheckAt = now;
				this.maybeScribe(sessionId);
			} catch (error) {
				this.log(error);
			}
		});
	...
	/** 从会话事件提取一行摘要（只取标量叶子字段；返回 null 表示无需记录）。 */
	scribeEventSummary(event) {
		...
		try {
			...
			const grab = (value) => {
				...
				if (value !== null && typeof value === "object") {
					for (const key of ["text", "message", "content", "name", "title"]) {
						if (value[key] !== void 0) {
							const got = grab(value[key]);
							if (got !== "") return got;
						}
					}
				}
			...
			if (text === "") {
				// 工具调用等无文本事件：只留类型标记
				if (/tool|job|subagent|workflow/.test(type)) return `[${type}]`;
				return null;
			}
			return `[${type}] ${text}`;
		} catch {
	...
	/** 从 ContentBlock[] 提取纯文本（子代理最终输出 lastAssistantMessage 等）。 */
	blockText(blocks) {
		...
		for (const block of blocks) {
			...
			else if (typeof block.content === "string" && block.content.trim() !== "") parts.push(block.content.trim());
		}
		return parts.join("\n");
	}

	/** 落后检测：树的更新时间落后于当前时间超过阈值且缓冲区有新事件时补记。 */
	maybeScribe(sessionId) {
		try {
			if (this.scribe.inFlight) return;
			const events = this.scribe.buffer.get(sessionId);
			if (events === void 0 || events.length === 0) return;
			const tree = this.store.treeForSession(sessionId);
			if (tree === null) return;
			const now = Date.now();
			const lag = now - new Date(tree.updatedAt).getTime();
			if (lag < this.scribe.lagMs) return; // 树还算新鲜，不打扰
			if (now - this.scribe.lastScribeAt < this.scribe.cooldownMs) return;
			// 只在会话绑定树存在且落后时补记
			this.scribe.inFlight = true;
			this.runScribe(sessionId, tree, events).catch((error) => {
				this.log(error);
			}).finally(() => {
				this.scribe.inFlight = false;
			});
		} catch (error) {
			this.log(error);
		}
	...
	/** 调 LLM 把事件压缩成树操作并应用；LLM 不可用则静默跳过。 */
	async runScribe(sessionId, tree, events) {
		const ctx = this.ctx;
		const llm = ctx.get?.("llm");
		if (llm === void 0) {
			this.scribe.buffer.delete(sessionId); // 无 LLM 能力，清缓冲避免重复尝试
			return;
		}
		const prompt = this.buildScribePrompt(tree, events);
		let text = "";
		try {
			const stream = llm.stream({
				provider: this.scribe.provider,
				model: this.scribe.model,
				messages: [
					{ role: "system", content: "你是研究进度树的记录员。根据给定的对话事件，输出 JSON 数组形式的树操作。只输出 JSON，不要任何解释。操作格式：{\"action\":\"start|conclude|annotate|plan-child\",\"title\":\"...\",\"desc\":\"...\",\"nodeId\":\"...\",\"status\":\"success|failed|abandoned|blocked\",\"reason\":\"...\"}。start 表示开启了新的探索分支（title 必填，写短标签≤12字）；conclude 结束一个分支（nodeId 填事件中提到的现有节点标题对应的 id 不可得时省略，仅当明确针对树中已有节点时提供，格式 {\"title\":\"节点标题\",\"status\":\"...\",\"reason\":\"...\"}）；annotate 补充说明（可带 desc）；plan-child 添加计划分支（title 短标签）。desc 写「在这个节点干了什么」（可长文本）；宁少勿错：不确定就输出 []。" },
					{ role: "user", content: prompt }
				]
			});
			for await (const chunk of stream) {
				const piece = chunk?.text ?? chunk?.delta ?? chunk?.content ?? null;
				if (typeof piece === "string") text += piece;
			}
		} catch (error) {
			this.log(`scribe llm failed: ${String(error)}`);
			return;
		}
		const ops = this.parseScribeOps(text);
		if (ops.length > 0) this.applyScribeOps(sessionId, ops);
		this.scribe.lastScribeAt = Date.now();
		this.scribe.buffer.delete(sessionId);
	}

	/** 构造补记提示：当前树摘要 + 新事件。 */
	buildScribePrompt(tree, events) {
		const nodeLines = tree.nodes.map((n) => `- ${n.id} | ${n.type} | ${n.status}${n.conclusion ? "/" + n.conclusion : ""} | ${n.title}`).slice(-60).join("\n");
		const eventLines = events.join("\n");
		return `当前研究主题：${tree.topic}\n\n当前树节点：\n${nodeLines}\n\n树上次更新后发生的事件：\n${eventLines}\n\n请输出 JSON 数组形式的树操作（见 system 说明）。`;
	}

	/** 解析 LLM 输出的 JSON 数组（容错：提取第一个 [...]）。 */
	parseScribeOps(text) {
		try {
			...
			return parsed.filter((op) => op !== null && typeof op === "object" && typeof op.action === "string");
		} catch {
			return [];
		}
	}

	/** 应用自动补记的操作（只允许安全的写操作，actor 标记 auto-scribe）。 */
	applyScribeOps(sessionId, ops) {
		const tree = this.store.treeForSession(sessionId);
		if (tree === null) return;
		for (const op of ops) {
			try {
				...
				if (action === "start") {
					...
					const node = {
						...
						title: String(op.title ?? "探索分支").slice(0, 120),
						desc: typeof op.desc === "string" && op.desc.trim() !== "" ? op.desc.trim().slice(0, 800) : null,
						status: "running",
						conclusion: null,
						reason: null,
						planRelation: null,
					...
					this.store.addNode(tree, node);
					if (parent !== null && parent.status === "pending") parent.status = "running";
					tree.currentNodeId = node.id;
				} else if (action === "conclude") {
					let node = null;
					if (typeof op.nodeId === "string" && op.nodeId !== "") node = this.store.nodeById(tree, op.nodeId);
					if (node === null && typeof op.title === "string" && op.title !== "") {
						node = tree.nodes.find((n) => n.status === "running" && n.title === op.title) ?? null;
					}
					if (node === null) continue;
					const status = String(op.status ?? "");
					if (!CONCLUSIONS.has(status)) continue;
					node.conclusion = status;
					node.reason = typeof op.reason === "string" ? op.reason.slice(0, 300) : node.reason ?? null;
					node.status = "ended";
					node.endedAt = node.endedAt ?? nowIso();
					this.store.settleAncestors(tree, node);
				} else if (action === "annotate") {
					const node = this.store.nodeById(tree, String(op.nodeId ?? ""));
					if (node === null) continue;
					if (typeof op.reason === "string" && op.reason !== "") node.reason = op.reason.slice(0, 300);
					if (typeof op.desc === "string" && op.desc.trim() !== "") node.desc = op.desc.trim().slice(0, 800);
				} else if (action === "plan-child") {
					const parent = this.store.attachParent(tree);
					const node = {
						id: shortId("plan"),
						type: "plan",
						parentId: parent === null ? null : parent.id,
						title: String(op.title ?? "计划分支").slice(0, 120),
						desc: typeof op.desc === "string" && op.desc.trim() !== "" ? op.desc.trim().slice(0, 800) : null,
						status: "pending",
						conclusion: null,
						reason: null,
				...
				this.log(`scribe op failed: ${String(error)}`);
			}
		}
		if (ops.length > 0) this.store.persist(tree);
	}
}
