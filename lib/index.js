	...
	async runScribe(sessionId, tree, events) {
		...
		try {
			const stream = llm.stream({
				...
				messages: [
					{ role: "system", content: "你是研究进度树的记录员。根据给定的对话事件，输出 JSON 数组形式的树操作。只输出 JSON，不要任何解释。操作格式：{\"action\":\"start|conclude|annotate|plan-child\",\"title\":\"...\",\"nodeId\":\"...\",\"status\":\"success|failed|abandoned|blocked\",\"reason\":\"...\"}。start 表示开启了新的探索分支（title 必填）；conclude 结束一个分支（nodeId 填事件中提到的现有节点标题对应的 id 不可得时省略，仅当明确针对树中已有节点时提供，格式 {\"title\":\"节点标题\",\"status\":\"...\",\"reason\":\"...\"}）；annotate 补充说明；plan-child 添加计划分支。宁少勿错：不确定就输出 []。" },
