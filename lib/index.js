...
const ACTIONS = new Set(["plan-root", "plan-child", "start", "conclude", "annotate", "plan-mark", "list", "resume"]);
...
class TreeStore {
	...
	/** 确保会话存在一棵树（按 goalId 或 session 键），并让会话指向它。 */
	ensureTree({ sessionId, goalId = null, topic = "" }) {
		const key = goalId !== null ? goalId : `session:${sessionId}`;
		...
		if (boundTree !== null && boundTree.shared === true && bound !== key) {
			return boundTree;
		}
...
class ResearchTreePlugin extends TypertRemoteService {
	...
	annotateNode(args) {
		...
		const tree = this.store.trees.get(treeId);
		if (tree === void 0) return { ok: false, error: `tree not found: ${treeId}` };
		const node = this.store.nodeById(tree, nodeId);
		if (node === null) return { ok: false, error: `node not found: ${nodeId}` };
		if (input.conclusion !== void 0 && input.conclusion !== null) {
			if (!CONCLUSIONS.has(String(input.conclusion))) return { ok: false, error: `invalid conclusion: ${String(input.conclusion)}` };
			node.conclusion = String(input.conclusion);
			if (input.reason !== void 0) node.reason = String(input.reason ?? "");
		...
		ctx.tools.register(defineTool({
			...
			parameters: {
				action: {
					...
					required: true,
					enum: [...ACTIONS],
					description: "操作：plan-root 建根(主题)；plan-child 加计划分支；start 开实际分支；conclude 记结论；annotate 补充说明；plan-mark 标记计划对照；list 列出全部树（续接检索用）；resume 把当前会话续接到已有树（新会话继续同一研究时用）。"
				},
				title: {
					type: "string",
				...
				treeId: {
					type: "string",
					description: "目标树 id（resume 必填；其他操作可选：显式指定本次操作作用在哪棵树）。"
				},
				nodeId: {
					type: "string",
	...
	subscribe(ctx) {
		...
		ctx.on("subagent/start", (info) => {
			try {
				...
				if (tree === null) return;
				const parentId = tree.currentNodeId ?? this.store.rootNode(tree)?.id ?? null;
				const node = {
					id: shortId("sub"),
					type: "subagent",
					parentId,
		...
		ctx.on("subagent/end", (info) => {
			try {
				const childId = String(info?.id ?? "");
				const record = this.activeSubagent.get(childId);
