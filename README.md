...
- **思维导图式渲染**：SVG 分层布局，**横向/竖向可切换**（⇄/⇅ 按钮，记忆选择；
  深链研究用竖向、宽分支用横向）；节点 = 短标题 + 一行说明
- **标题与描述分层**：`title` 是短标签（节点上截断 11 字），`desc` 是「在这个节点干了什么」
  （长文本，悬停/点击详情看全文），`reason` 是结论原因——三件事分开写，标题不再又长又挤
...
- **详情**：悬停节点看详细说明（说明/结论/原因/时间/产出文件），点击固定；
  **固定卡片可 ✎ 直接编辑标题/描述**（保存即写回，乐观更新，无需改 JSON）
- **跳转**：双击节点跳转到其所属会话
- **自动骨架**：goal 轮次、子代理、工作流自动挂到树上；
  **子代理节点自动补全内容**（任务 = 子会话首条消息，结果 = 最终输出截取），不再空壳
